...
echo "$($PSQL "SELECT COUNT(game_id) FROM games FULL JOIN teams ON games.winner_id = teams.team_id WHERE winner_goals > 2")"
...
echo "$($PSQL "SELECT name FROM teams FULL JOIN games ON teams.team_id = games.winner_id WHERE round = 'Eighth-Final' AND year = 2014 UNION SELECT name FROM teams FULL JOIN games ON teams.team_id = games.opponent_id WHERE round = 'Eighth-Final' AND year = 2014 ORDER BY name ASC")"
